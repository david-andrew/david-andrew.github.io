...
export const Checkbox = ({ label, isChecked, onChange, className }: CheckboxProps) => {
    return (
        <div className={twMerge('flex items-center', className)}>
            <input type="checkbox" id="custom-checkbox" className="hidden" checked={isChecked} onChange={onChange} />
            <label
...
export const Container = ({ children }: { children: React.ReactNode }) => {
    return <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-[1190px]">{children}</div>
}

export const Divider = ({ className = '' }: { className?: string }) => {
    return <hr className={twMerge('h-px my-4 bg-gray-500 border-0', className)} />
}
...
export const Link = ({
    ...
    return (
        <NextLink
            href={href}
            className={twMerge('text-blue-400 hover:text-blue-500 font-gentona text-xl', className)}
            {...props}
        >
...
export const HorizontalScroll = ({
    ...
    return (
        <div className={twMerge('flex flex-row overflow-y-hidden overflow-x-auto', className)}>
            {' '}
            {/* hide-h-scrollbar */}
            {children}
        </div>
