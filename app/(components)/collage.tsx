    ...
    return (
        <>
            ...
            <div className="hidden xl:block 2xl:hidden">
                <CollagePage
                    images={images}
                    rowSizes={reflowable ? rowSizes.map((rowSize) => Math.max(1, rowSize - 1)) : rowSizes}
            ...
            <div className="hidden lg:block xl:hidden">
                <CollagePage
                    images={images}
                    rowSizes={reflowable ? rowSizes.map((rowSize) => Math.max(1, rowSize - 2)) : rowSizes}
                    clickHandler={clickHandler}
                />
            </div>
            <div className="hidden md:block lg:hidden">
                <CollagePage
                    images={images}
                    rowSizes={reflowable ? rowSizes.map((rowSize) => Math.max(1, rowSize - 3)) : rowSizes}
                    clickHandler={clickHandler}
                />
            </div>
            <div className="hidden sm:block md:hidden">
                <CollagePage
                    images={images}
                    rowSizes={reflowable ? rowSizes.map((rowSize) => Math.max(1, rowSize - 4)) : rowSizes}
                    clickHandler={clickHandler}
                />
            </div>
            <div className="sm:hidden">
                <CollagePage
                    images={images}
                    rowSizes={reflowable ? rowSizes.map((_) => 1) : rowSizes}
                    clickHandler={clickHandler}
                />
            </div>
...
const getLayout = (
    images: StaticImageData[],
    rowSizes: number[] = [3, 4, 5],
): { image: StaticImageData; idx: number }[][] => {
    const layout: number[] = []
    {
...
export const SingleImageCollage = ({
    image,
    className = '',
}: {
    image: StaticImageData
