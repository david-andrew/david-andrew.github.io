...
const splitParserOutput = (raw?: string): ParserOutput | undefined => {
    ...
    const endArrows = '<<<<<<<<<<<<'
    const [metascanner, metaast, metaparser, grammar, table, result, forest] = raw.split(`${endArrows}`) as (
        | string
        | undefined
    )[]
...
const AutoHeightTextArea = ({
    ...
    return (
        <textarea
            ref={textAreaRef}
            className={twMerge(
                'hide-h-scrollbar overflow-y-hidden rounded-md font-mono resize-none overflow-x-auto whitespace-pre outline outline-1 outline-[#808080] focus:outline-2 focus:outline-white px-0.5',
                className,
            )}
...
export const DewyLiveParser = ({ grammars, initial_idx = 0 }: { grammars: DemoGrammar[]; initial_idx?: number }) => {
    if (grammars.length < initial_idx) {
        throw new Error(
            `initial_idx must be less than the number of grammars. initial_idx: ${initial_idx}, grammars.length: ${grammars.length}`,
        )
    }
    ...
    const setGrammarText = (text: string) => {
        _setGrammarText(text)
        if (!demoStarted) setDemoStarted(true)
    }
    ...
    return (
        <>
            ...
            <div className={twMerge('max-md:hidden grid w-full', gap, columns)}>
                ...
                <AutoHeightTextArea
                    className={twMerge(
                        'w-full bg-[#232323] text-xl',
                        parseError ? 'outline-[#FF0000] focus:outline-[#FF0000]' : '',
                    )}
                    text={sourceText}
                ...
                <CodeEditor
                    className={twMerge(
                        'w-full bg-[#232323] text-xl',
                        grammarError ? 'outline outline-[#FF0000] focus:outline-[#FF0000]' : '',
                    )}
            ...
            <div className="md:hidden flex flex-col">
                ...
                <CodeEditor
                    className={twMerge(
                        'w-full bg-[#232323] text-lg hide-h-scrollbar overflow-y-hidden overflow-x-auto',
                        grammarError ? 'outline outline-[#FF0000] focus:outline-[#FF0000]' : '',
                    )}
                    language={dewy_meta_lang()}
                    theme={dewy_meta_theme}
            ...
            {!demoStarted && (
                <div className="flex flex-col justify-center pt-6">
                    <button
                        className="w-24 h-8 bg-[#232323] text-white rounded-md"
